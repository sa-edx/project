...
function readFilesAsDataUrls(fileList) {
  return Promise.all(
    Array.from(fileList || []).map(
      (file) =>
        new Promise((resolve) => {
          readFileAsDataUrl(file, (dataUrl) => resolve(dataUrl));
        }),
    ),
  );
}
...
function ContextPanel({ project, selectedBuilding, selectedFloor, selectedUnit, stats, onDeleteBuilding }) {
  return (
    <aside className="structure-context panel">
      ...
      <div className="structure-context__stack">
        ...
        <div className="structure-context__card">
          ...
          {selectedBuilding ? (
            <button type="button" className="ghost danger" onClick={() => onDeleteBuilding?.(selectedBuilding.id)}>
              Delete building
            </button>
          ) : null}
        </div>
...
export default function StructureManagerV2({
  ...
  const [unitGalleryUrl, setUnitGalleryUrl] = useState('');
  const [unitEditGalleryUrl, setUnitEditGalleryUrl] = useState('');
  ...
  useEffect(() => {
    if (selectedUnit) {
      setUnitEditForm({
        ...
        layoutPlan: selectedUnit.layoutPlan || '',
        ...
        <form className="stack" onSubmit={(event) => { event.preventDefault(); onCreateUnit(); }}>
          ...
          <div className="media-upload">
            <div className="panel-head panel-head--compact">
              <h4>Layout Plan</h4>
              <p>Upload or paste the floor layout image for this unit.</p>
            </div>
            <input
              placeholder="Unit layout plan URL or data URL"
              value={unitForm.layoutPlan}
              onChange={(e) => setUnitForm((c) => ({ ...c, layoutPlan: e.target.value }))}
            />
            <input
              type="file"
              accept="image/*"
              onChange={(e) => {
                const file = e.target.files?.[0];
                if (file) {
                  readFileAsDataUrl(file, (dataUrl) => setUnitForm((c) => ({ ...c, layoutPlan: dataUrl })));
                }
              }}
            />
            {isImageSource(unitForm.layoutPlan) ? (
              <div className="floor-plan-preview floor-plan-preview--compact">
                <img src={unitForm.layoutPlan} alt="Unit layout preview" />
              </div>
            ) : null}
          </div>
          <div className="media-upload">
            <div className="panel-head panel-head--compact">
              <h4>Gallery Views</h4>
              <p>Add the unit gallery images that should appear in the public viewer.</p>
            </div>
            <div className="media-upload__controls">
              <input
                type="text"
                placeholder="Paste gallery image URL and add it"
                value={unitGalleryUrl}
                onChange={(e) => setUnitGalleryUrl(e.target.value)}
              />
              <button
                type="button"
                className="ghost"
                onClick={() => {
                  const nextUrl = unitGalleryUrl.trim();
                  if (!nextUrl) {
                    return;
                  }
                  setUnitForm((c) => ({
                    ...c,
                    media: parseList(c.media).concat(nextUrl).join('\n'),
                  }));
                  setUnitGalleryUrl('');
                }}
              >
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                if (files?.length) {
                  ...
                  setUnitForm((current) => ({
                    ...current,
                    media: [...parseList(current.media), ...images].join('\n'),
                  }));
                  e.target.value = '';
                }
              }}
            ...
            <textarea
              className="text-area"
              placeholder="Unit gallery view image URLs, one per line"
              rows="3"
              value={unitForm.media}
              onChange={(e) => setUnitForm((c) => ({ ...c, media: e.target.value }))}
            />
            <div className="media-upload__grid">
              {parseList(unitForm.media).map((item, index) =>
                isImageSource(item) ? (
                  <img key={`${item}-${index}`} src={item} alt={`Gallery view ${index + 1}`} />
                ) : (
                  <a key={`${item}-${index}`} href={item} target="_blank" rel="noreferrer">
                    View {index + 1}
                  </a>
                ),
              )}
      ...
      <div className="manager-column">
        ...
        <form className="stack" onSubmit={(event) => { event.preventDefault(); if (selectedUnitId) onUpdateUnit(selectedUnitId, { ...unitEditForm, bedrooms: Number(unitEditForm.bedrooms || 0), bathrooms: Number(unitEditForm.bathrooms || 0), area: unitEditForm.area === '' ? null : Number(unitEditForm.area), basePrice: unitEditForm.basePrice === '' ? null : Number(unitEditForm.basePrice), layoutPlan: unitEditForm.layoutPlan || null, media: parseList(unitEditForm.media), amenities: unitEditForm.amenities }); }}>
          ...
          <div className="media-upload">
            <div className="panel-head panel-head--compact">
              <h4>Layout Plan</h4>
              <p>Update the floor layout image for this unit.</p>
            </div>
            <input
              placeholder="Unit layout plan URL or data URL"
              value={unitEditForm.layoutPlan}
              onChange={(e) => setUnitEditForm((c) => ({ ...c, layoutPlan: e.target.value }))}
            />
            <input
              type="file"
              accept="image/*"
              onChange={(e) => {
                const file = e.target.files?.[0];
                if (file) {
                  readFileAsDataUrl(file, (dataUrl) => setUnitEditForm((c) => ({ ...c, layoutPlan: dataUrl })));
                }
              }}
            />
            {isImageSource(unitEditForm.layoutPlan) ? (
              <div className="floor-plan-preview floor-plan-preview--compact">
                <img src={unitEditForm.layoutPlan} alt="Unit layout preview" />
              </div>
            ) : null}
          </div>
          <div className="media-upload">
            <div className="panel-head panel-head--compact">
              <h4>Gallery Views</h4>
              <p>Update the unit gallery images shown in the public viewer.</p>
            </div>
            <div className="media-upload__controls">
              <input
                type="text"
                placeholder="Paste gallery image URL and add it"
                value={unitEditGalleryUrl}
                onChange={(e) => setUnitEditGalleryUrl(e.target.value)}
              />
              <button
                type="button"
                className="ghost"
                onClick={() => {
                  const nextUrl = unitEditGalleryUrl.trim();
                  if (!nextUrl) {
                    return;
                  }
                  setUnitEditForm((c) => ({
            ...
            />
            <div className="media-upload__grid">
              {parseList(unitEditForm.media).map((item, index) =>
                isImageSource(item) ? (
                  <img key={`${item}-${index}`} src={item} alt={`Gallery view ${index + 1}`} />
                ) : (
                  <a key={`${item}-${index}`} href={item} target="_blank" rel="noreferrer">
