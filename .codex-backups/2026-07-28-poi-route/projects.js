...
function serializeProject(project) {
  return {
    ...
    model3dUrl: project.model3dUrl || null,
...
projectsRouter.put(
  ...
  express.raw({ type: ['application/octet-stream', 'model/gltf-binary', 'model/gltf+json', '*/*'], limit: '100mb' }),
  async (req, res, next) => {
    try {
      const project = await prisma.project.findUnique({
        where: { id: req.params.id },
      ...
      const originalName = String(req.get('x-file-name') || req.get('x-filename') || req.query.fileName || '');
      const extension = getModelFileExtension(originalName, req.get('content-type'));
      const fileName = `${project.projectCode || 'project'}-${project.id}-${crypto.randomUUID()}${extension}`;
      ...
      return res.json({ data: serializeProject(updated) });
    } catch (error) {
      next(error);
    }
  },
