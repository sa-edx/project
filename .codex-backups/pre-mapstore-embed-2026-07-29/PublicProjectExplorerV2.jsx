...
function collectShowcaseImages(projects) {
  const items = [];
...
export default function PublicProjectExplorerV2({
  ...
  const showcaseImages = useMemo(() => collectShowcaseImages(filteredProjects.length ? filteredProjects : projects), [filteredProjects, projects]);
        ...
        <section className="panel public-showcase">
          <div className="public-showcase__frame">
            {showcaseImages.length ? (
              <div className="public-showcase__track" aria-label="Project image showcase">
                {[...showcaseImages, ...showcaseImages].map((item, index) => (
                  <article className="public-showcase__card" key={`${item.id}-${index}`}>
                    <img src={item.src} alt={item.title} className="public-showcase__image" loading="lazy" />
              ...
              <div className="empty-state">
                <h3>No project images yet</h3>
                <p>Once users upload cover images or gallery images, they will appear here in a scrolling showcase.</p>
              </div>
            )}
          </div>
        </section>
